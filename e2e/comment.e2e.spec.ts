...
test('POST should return error if token does not exist', async () => {
  const response = await supertest(app).post('/comment').send(data);
...
test('POST should return error if postId does not exist', async () => {
  const token = process.env.TOKEN;
  const response = await supertest(app)
    .post('/comment')
    .set('authorization', `Bearer ${token}`)
    .send(data);
  expect(response.body.msg).toBe('post id not found');
  expect(response.status).toBe(400);
});
